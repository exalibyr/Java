...
public class SimpleXORModule extends AbstractModule{
    ...
    public SimpleXORModule(JPanel panel){
        ...
        encodedMessage.setPreferredSize(new Dimension(100, 25));
        panel.add(encodedMessage);

        decodedMessage = new JLabel("decoded message");
        decodedMessage.setPreferredSize(new Dimension(100, 25));
        panel.add(decodedMessage);
    }

    protected void calculate(){
        String result[] = Logic.xor(message.getText(), key.getText());
        encodedMessage.setText(result[0]);
        decodedMessage.setText(result[1]);
    }
}
